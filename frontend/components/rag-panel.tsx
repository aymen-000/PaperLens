...
export function RAGPanel({ paper, onClose }: RAGPanelProps) {
  ...
  return (
    <div className="fixed inset-0 md:right-0 md:left-auto md:w-96 bg-background border-l border-border shadow-lg z-50 flex flex-col">
      {/* Header */}
      <div className="flex-shrink-0 border-b border-border p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <FileText className="w-5 h-5 text-primary" />
            <h2 className="text-lg font-semibold">Paper Q&A</h2>
          </div>
          <Button
            variant="ghost"
            size="sm"
        ...
        <div className="text-sm text-muted-foreground">
          <p className="font-medium text-foreground truncate mb-1">{paper.title}</p>
          <p>
                    ...
                    <div className="mt-3 pt-2 border-t border-border/30">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {message.references.map((ref, index) => (
                          <span key={index} className="text-xs bg-background/50 px-2 py-1 rounded border">
                            {ref}
                          </span>
                        ))}
                      </div>
